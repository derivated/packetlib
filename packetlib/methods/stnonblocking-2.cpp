    ...
    int execute_requests(
        ...
        if (curl) {
            for (int i = 0; i < count; i++) {
                ...
                // Set HTTP version to 2 and enable TFO
            ...
            curl_multi_setopt(curl, CURLMOPT_MAX_HOST_CONNECTIONS, count);
            curl_multi_setopt(curl, CURLMOPT_MAX_TOTAL_CONNECTIONS, count);
